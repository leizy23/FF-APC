...
%节点对应索引
index_FPZ = gen(:,1);
index_Vtheta = gen(end,1);% 提供参考相角 包含关系
...
%获取减去负荷后的发电量作为最大功率
Pref_SG = real(S_gen(index_FPZ)); %获取减去负荷后的发电量作为最大功率
P_load_SG = real(S_load(index_FPZ));
Kp = ones(length(index_FPZ),1);
J = 5*ones(length(index_FPZ),1)+2*rand(length(index_FPZ),1);
D = 0.5*ones(length(index_FPZ),1)+0.4*rand(length(index_FPZ),1);
...
% 获取所有发电机节点的电压幅值
V_m([index_FPZ],1) = bus([index_FPZ],VM);
...
x0(1:m1-1,1) = bus(index_FPZ(1:end-1,1),VA)*pi/180;
x0(m1:2*m1-1,1) = omega0;
...
[sep,fval] = fsolve('SG_dae',x0,options);
% sep = x0;
norm(SG_dae(sep))
if norm(SG_dae(sep)) > 1e-6
